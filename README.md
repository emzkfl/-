...
보스 능력치 데이터는 `data/boss-rules.json`에 분리되어 있습니다. 현재 보스 레벨, 체력, 방어율, 제한시간, 포스 요구량은 나무위키 `메이플스토리/보스 몬스터` 하위 보스 문서 기준으로 기록하며, `/api/formulas`의 `bossRules`와 `bossRuleSource`에서도 확인할 수 있습니다.
...
요구 환산 = 기준 요구 환산 * sqrt(hpRatio * 30 / 20)
...
- 제한시간/체력 보정: `hpRatio`는 30분 기준 대비 현재 보스 체력/제한시간 비율입니다. 예를 들어 20분 기준으로 체력이 조정된 보스는 `0.6667`, 30분 보스는 `1.0`, 검은 마법사처럼 60분 기준 보스는 `2.0`으로 둡니다.
